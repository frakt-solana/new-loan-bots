...
export const postTweet = async ({
  ...
  try {
    ...
    const imageData = await readFile(fullPathToCardImage, { encoding: 'base64' });
    ...
    console.error('Error posting image on Twitter: ', error);
  }
}
...
const generateTwitterPostText = async ({
  ...
  try {
    const allMessageTemplates = await (await fetch(MESSAGE_TEMPLATES_JSON_URL)).json();
    ...
    const messageTemplates = nftCollectionName
      ? allMessageTemplates
      : allMessageTemplates.filter((message) => !message.includes('{nftCollectionName}'));

    const messageTemplatesForLoanTerm = allMessageTemplates.filter((message) => !message.includes('{period}'));
